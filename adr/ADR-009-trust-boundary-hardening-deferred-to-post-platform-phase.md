...
## Post-STORY-012 update: the hardening phase itself has started

At the user's direction, this phase started — see [ADR-011](ADR-011-real-operator-authentication-first-slice.md) for the first slice: real operator authentication (login + signed sessions) applied to `POST /requests/:id/approve` and `POST /requests/:id/deny` specifically. **This closes item 2 (verified approver identity) for those two routes only.** Item 1 (caller authentication on every route) is not closed — every other mutating route remains open. Items 3 (token possession proof) and 4 (real persistence) are untouched. The Decision section above still accurately describes the overall bundle; this note exists so the ADR reflects that the phase is genuinely underway, not still purely theoretical.

## Post-STORY-012 update: second slice — agent/API-caller authentication

See [ADR-012](ADR-012-agent-caller-authentication-second-slice.md) for the full design: `POST /requests` now requires a real, registered agent credential (`Authorization: Bearer <id>.<secret>`), and `subject` is derived from that authentication instead of being read from the request body. **This closes item 1 (real caller authentication) for `POST /requests` only** — the route where an unverified `subject` did the most damage, since it's the field every downstream policy decision keys off. Every other mutating route named in the original Context section above — `POST /tokens/:id/revoke`, `POST /policies`, `PATCH /policies/:id`, `POST /redaction-rules`, `POST /tokens/:id/delegate` — remains completely unauthenticated. Item 3 (token possession proof) and item 4 (real persistence) are untouched, exactly as scoped when the phase started. Two slices in, the shape holding is: each slice closes one route's worth of one item, named precisely, with the rest of the bundle stated plainly as still open rather than implied closed by the phase's title.

## Post-STORY-012 update: third slice — token possession proof, and the hardening phase's original three items close
