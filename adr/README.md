...
| [015](ADR-015-control-hardening-first-slice.md) | Control hardening, first slice — `POST /policies`, `PATCH /policies/:id`, `POST /redaction-rules`, `POST /tokens/:id/revoke` now require a real operator session; revocation gained real actor attribution. Rate limiting (second slice) not yet built | Post-STORY-012, INPACT trust scorecard |
