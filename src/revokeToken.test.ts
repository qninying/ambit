...
describe("revokeToken", () => {
  ...
  it("makes the very next enforceToken call by id fail, right after revocation", () => {
  ...
  it("blocks the action, not just reports a decision", () => {
    const { store, auditLog } = setup();
    ...
    expect(enforceToken(token.id, "payment:charge", store, auditLog).allowed).toBe(false);
  });

  // Acceptance: "Trust: Revocation is logged with a reason code."
  it("logs the revocation itself with the given reason code", () => {
    const { store, auditLog } = setup();
    const token = issueToken({ subject: "agent-42", scope: ["email:send"], ttlSeconds: 300 }, store);
  ...
  it("updates the token's own status to revoked in the store", () => {
    const { store, auditLog } = setup();
    const token = issueToken({ subject: "agent-42", scope: ["email:send"], ttlSeconds: 300 }, store);
